...
% Title information
\title{Multisets in Discrete Mathematics}
\subtitle{Combinations with Repetition - M344}
...
\begin{frame}{What is a Multiset?}
  \begin{block}{Formal Definition}
    A \textbf{multiset} (or \textbf{bag}) is a generalization of a set where elements can appear more than once. 
    
    Formally, a multiset $M$ over a set $S$ is a function $m: S \to \mathbb{N}_0$ where $m(x)$ denotes the \textbf{multiplicity} of element $x$ in $M$.
  \end{block}
  
  \vspace{0.3cm}
  
  \begin{block}{Notation and Examples}
    \begin{itemize}
      \item \textbf{Multiset notation}: $\{a, a, b, c, c, c\}$ or $\{a^2, b, c^3\}$
      \item \textbf{Cardinality}: $|M| = \sum_{x \in S} m(x)$ (total number of elements counting multiplicities)
      \item \textbf{Example}: $M = \{1^3, 2^2, 5\}$ means: 1 appears 3 times, 2 appears 2 times, 5 appears once
    \end{itemize}
  \end{block}
  
  \begin{alertblock}{Key Property}
    Order does NOT matter: $\{a, a, b\} = \{a, b, a\} = \{b, a, a\}$
  \end{alertblock}
\end{frame}

\begin{frame}{Multisets in Discrete Mathematics}
  \begin{block}{Applications in Discrete Math}
    \begin{itemize}
      \item \textbf{Combinatorics}: Counting with repetition allowed
      \item \textbf{Number Theory}: Integer partitions and compositions
      \item \textbf{Generating Functions}: Coefficient extraction in power series
      \item \textbf{Graph Theory}: Degree sequences (multisets of vertex degrees)
      \item \textbf{Algorithm Analysis}: Analyzing data structures with duplicates
      \item \textbf{Recurrence Relations}: Solutions to linear homogeneous recurrences
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}{Multisets vs Combinations vs Permutations}
  \begin{block}{Counting Classification}
    \begin{tabular}{lllll}
      \toprule
      \textbf{Type} & \textbf{Order?} & \textbf{Repetition?} & \textbf{Formula} & \textbf{Example} \\
      \midrule
      Permutations & Yes & No & $P(n,r) = \frac{n!}{(n-r)!}$ & $n=4,r=2: 12$ \\
      Combinations & No & No & $\binom{n}{r} = \frac{n!}{r!(n-r)!}$ & $n=4,r=2: 6$ \\
      Perm. w/ Rep & Yes & Yes & $n^r$ & $n=4,r=2: 16$ \\
      \textbf{Multisets} & \textbf{No} & \textbf{Yes} & $\binom{n+r-1}{r}$ & $n=4,r=2: 10$ \\
      \bottomrule
    \end{tabular}
  \end{block}
  ...
  \begin{exampleblock}{Discrete Math Context}
    \begin{itemize}
      \item \textbf{Problem}: Choose $r$ objects from $n$ types with repetition, order irrelevant
      \item \textbf{Notation}: $\left(\!\!\binom{n}{r}\!\!\right)$ called "n multichoose r"
      \item \textbf{Read as}: "Combinations of $n$ types taken $r$ at a time with repetition"
    \end{itemize}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Problem 1: Multiset Cardinality and Subsets}
  \begin{block}{Problem}
    Let $M = \{a^3, b^2, c^4\}$ be a multiset.
    
    \begin{enumerate}
      \item What is the cardinality $|M|$?
      \item How many multisets can be formed from $M$ (including the empty multiset and $M$ itself)?
      \item How many sub-multisets of $M$ have cardinality exactly 5?
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}{Problem 2: Basic Counting with Repetition}
  \begin{block}{Problem}
    Using the multiset formula $\left(\!\!\binom{n}{r}\!\!\right) = \binom{n+r-1}{r}$:
    
    \begin{enumerate}
      \item Calculate $\left(\!\!\binom{5}{3}\!\!\right)$
      \item How many multisets of cardinality 6 can be formed from a set of 4 elements?
      \item Verify that $\left(\!\!\binom{n}{r}\!\!\right) = \binom{n}{r}$ is NOT generally equal
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}{Problem 3: Non-negative Integer Solutions}
  \begin{block}{Problem}
    ...
    Explain why this is equivalent to finding multisets of cardinality 12 from a set of 4 elements.
  \end{block}
\end{frame}
...
\begin{frame}{Problem 4: Weak Compositions}
  \begin{block}{Problem}
    A \textbf{weak composition} of $n$ into $k$ parts is a sequence $(a_1, a_2, \ldots, a_k)$ where $a_i \geq 0$ and $\sum a_i = n$.
    
    \begin{enumerate}
      \item How many weak compositions of 10 into 4 parts exist?
      \item Compare this with the number of compositions (where each $a_i > 0$)
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}{Problem 5: Distribution and Stars-Bars}
  \begin{block}{Problem}
    \begin{enumerate}
      \item In how many ways can 8 identical objects be distributed into 3 distinct boxes?
      \item Express this problem using stars and bars notation
      \item What if each box must contain at least one object?
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}{Problem 6: Positive Integer Solutions and Compositions}
  \begin{block}{Problem}
    Find the number of \textbf{positive} integer solutions to:
    $$x_1 + x_2 + x_3 = 10$$
    
    \begin{enumerate}
      \item Use the substitution method ($y_i = x_i - 1$)
      \item Explain why this counts the number of \textbf{compositions} of 10 into 3 parts
      \item Compare with weak compositions
    \end{enumerate}
  \end{block}
\end{frame}
